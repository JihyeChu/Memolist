	...
	<form action="updateOK.jsp" method="post">
		<table width="700" align="center" border="1" cellpadding="5" cellspacing="0">
			...
			<tr>
				<th>메모</th>
				<td> 
					<input type="text" name="memo" value="<%=rs.getString("memo")%>" size="70"/>
				</td>
			</tr>
			<tr>
...
<%			
				...
				// 수정할 글이 테이블에 존재하지 않기 때문에 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
				out.println("<script>");
			...
			// 수정할 글번호와 돌아갈 페이지 번호가 모두 숫자가 아니므로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
			out.println("<script>");
			out.println("alert('수정할 글번호 또는 돌아갈 페이지 번호가 숫자가 아닙니다.')");
			out.println("location.href='memoList4.jsp'");
			out.println("</script>");
		}
	} else {
		// 수정할 글번호와 돌아갈 페이지 번호가 모두 넘어오지 않았으므로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
		out.println("<script>");
