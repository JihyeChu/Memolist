...
//	삭제할 글번호와 삭제 후 돌아갈 페이지 번호가 모두 넘어왔나 검사한다.
	if (tempIdx != null && tempCurrentPage != null && tempIdx.trim().length() != 0 && tempCurrentPage.trim().length() != 0) {
		...
		try {
			...
			// 삭제할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기 위해서 글번호에 해당되는 글을 얻어온다.
...
<%			
				...
				// 삭제할 글이 테이블에 존재하지 않기 때문에 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
				out.println("<script>");
			...
			// 삭제할 글번호와 돌아갈 페이지 번호가 모두 숫자가 아니므로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
			out.println("<script>");
			out.println("alert('삭제할 글번호 또는 돌아갈 페이지 번호가 숫자가 아닙니다.')");
			out.println("location.href='memoList4.jsp'");
			out.println("</script>");
		}
	} else {
		// 삭제할 글번호와 돌아갈 페이지 번호가 모두 넘어오지 않았으므로 에러 메시지를 출력하고 memoList4.jsp로 보낸다.
		out.println("<script>");
